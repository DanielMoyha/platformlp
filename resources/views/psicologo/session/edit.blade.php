...
@section('content')
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item active">Editar algunos detalles de la sesión</li>
    </ol>

    <div class="card mb-4 border-warning shadow">
        <div class="card-header bg-warning text-center text-white fw-bold h5">Modificar Sesión</div>
        <div class="card-body">
            <div class="row border-bottom">
                <div class="col-md-6 mb-3">
                    <span class="form-label">Paciente: {{ $sesion->hijo->nombre }}</span>
                </div>
                <div class="col-md-6 mb-3">
                    <span class="form-label">Número de Caso: {{ $sesion->hijo->paciente->caso }}</span>
                </div>
            </div>
            <div class="row mt-3">
                <form action="{{ route('session.update', [$sesion->id]) }}" class="row needs-validation justify-content-around" novalidate method="POST">
                    @csrf
                    @method('put')
                    <input type="hidden" value="{{ $sesion->hijo->id }}" name="hijo_id">
                    ...
                    <div class="d-flex justify-content-end border-top">
                        <a href="{{ route('session') }}" class="btn btn-danger fw-bold mx-1 mx-md-2 mt-3">Cancelar</a>
                        <button class="btn btn-success fw-bold mt-3" type="submit">Actualizar Sesión</button>
                    </div>
                </form>
            </div>
        </div>
